...
def drop_keys(configuration: Dict[str, Any], key_blacklist: Tuple[str, ...]):
    return {
        key: value for key, value in configuration.items() if key not in key_blacklist
    }
...
class Argument(typing.NamedTuple):
    ...
    def serialize(self, *, isolation_mode: bool = False) -> Dict[str, Any]:
        ...
        qualifiers = JSON_QUALIFIER_TO_OPTIONS[
            configuration.get("nargs", Qualifiers.SUPPRESS)
        ]
...
JSON_DIRECT_MIRROR_OPTIONS = ("choices", "metavar")
