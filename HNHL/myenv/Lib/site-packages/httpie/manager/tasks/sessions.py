...
def upgrade_session(
    env: Environment, args: argparse.Namespace, hostname: str, session_name: str
):
    ...
    env.stdout.write(
        f"Upgraded {session_name!r} @ {hostname!r} to v{session.version}\n"
    )
    return ExitStatus.SUCCESS
...
def cli_upgrade_session(env: Environment, args: argparse.Namespace) -> ExitStatus:
    return upgrade_session(
        env, args=args, hostname=args.hostname, session_name=args.session
    )
    ...
    for host_path in session_dir_path.iterdir():
        ...
        for session_path in host_path.glob("*.json"):
            ...
            status |= upgrade_session(
                env, args=args, hostname=hostname, session_name=session_name
            )
    return status
