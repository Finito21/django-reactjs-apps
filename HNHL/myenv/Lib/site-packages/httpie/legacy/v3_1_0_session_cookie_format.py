...
def pre_process(session: "Session", cookies: Any) -> List[Dict[str, Any]]:
    ...
    should_issue_warning = is_old_style and any(
        cookie.get("domain", "") == "" for cookie in normalized_cookies
    )

    if should_issue_warning:
        warning = INSECURE_COOKIE_JAR_WARNING.format(
            hostname=session.bound_host, session_id=session.session_id
        )
...
def post_process(
    normalized_cookies: List[Dict[str, Any]], *, original_type: Type[Any]
) -> Any:
...
def fix_layout(session: "Session", hostname: str, args: argparse.Namespace) -> None:
    ...
    session["cookies"] = [
        {"name": key, **value} for key, value in session["cookies"].items()
    ]
