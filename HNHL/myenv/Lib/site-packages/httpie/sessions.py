...
def materialize_cookie(cookie: Cookie) -> Dict[str, Any]:
    materialized_cookie = {
        option: getattr(cookie, option) for option in KEPT_COOKIE_OPTIONS
    }
...
def materialize_headers(headers: Dict[str, str]) -> List[Dict[str, Any]]:
    return [{"name": name, "value": value} for name, value in headers.copy().items()]
...
class Session(BaseConfigDict):
    ...
    def post_process_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        for key, store, serializer, exporter in [
            ...
            data[key] = exporter(values, original_type=original_type)
    ...
    def warn_legacy_usage(self, warning: str) -> None:
        ...
        self.env.log_error(warning, level=LogLevel.WARNING)
